...
def result():
    ...
    time.sleep(0.5)
    driver.find_element_by_xpath('//*[@id="endDate"]').send_keys("01/01/2022")
    time.sleep(0.5)
...
#print(list_of_dfs)
    ...
    if len(list_of_dfs) == 387:
        table = list_of_dfs[385]
        table = table.fillna('')
        print(table)
        table.to_excel("latest_PE2.xlsx", index=False)
    elif len(list_of_dfs) == 388:    
        table = list_of_dfs[386]
        table = table.fillna('')
        print(table)
        table.to_excel("latest_PE2.xlsx", index=False)
            
    return render_template('index.html', cond = cond)
